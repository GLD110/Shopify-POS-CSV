...
class Product extends MY_Controller {
  ...
  public function update_pos()
  {
    ...
    if(isset( $_GET[ "file_name" ]))
    {
      ...
      $action = 'products/count.json';
      $countInfo = $this->Shopify_model->accessAPI( $action, '', 'GET' );
      $count = $countInfo->count;
      $pages = (int)( $count / 250 ) + 1;
      $order = 0;

      set_time_limit(0);

      for($page=1; $page<$pages+1; $page++){
        $action = 'products.json?' . 'limit=250&page=' . $page;
        $productInfo = $this->Shopify_model->accessAPI( $action, '', 'GET' );
      foreach($productInfo->products as $product){
      foreach($product->variants as $variant){
      $UPC = $ekey_products[$order]['UPC'];
      $order = $order + 1;

      $action = 'products/' . $variant->product_id . '.json';
      ...
      $U_productInfo = $this->Shopify_model->accessAPI( $action, $products_array, 'PUT' );

      if(!isset($U_productInfo->product)){
        var_dump("error" . '-' . $U_productInfo->errors->product);
      }
      else{
        var_dump("success" . '-' . $U_productInfo->product->handle);
      }
      }}}
      echo "POS Updated";
    }
